...
|------------|---------------------------------------|-------------------------------------------|-----------------------------------------------|
| 구조        | Controller → Service → Repository 계층 | 도메인 중심, 외부 입출력 분리                    | 도메인 모델 중심 계층 분리                          |
| 의존성 방향   | 하향식                                  | 도메인 중심                                  | 내부 방향 (외부 -> 내부)                          |
| 외부 의존성   | 직접 의존                                | 추상화를 통한 간접 의존                        | 포트와 어댑터를 통한 철저한 분리                      |
| 개발 속도     | 초기 개발이 빠르지만 장기적 유지보수 비용 증가   | 초기 개발이 느리지만 비즈니스 복잡성 증가에 따른 유지보수 비용 감소 | 초기 설계에 시간이 들지만 외부 의존성 교체 용이성으로 유지보수 비용 감소 |
| 테스트 용이성  | ?                         | ?                                       | ?                                           |
