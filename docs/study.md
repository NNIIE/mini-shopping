...
# 정규화
## 핵심 원리
- 각 테이블은 정확히 하나의 주제만 다뤄야 한다.
- 테이블 간 중복 데이터를 허용하지 않음으로써 무결성 유지
...
### 제2정규형(2NF)
1NF이면서 테이블의 모든 컬럼이 완전 함수적 종속을 만족하도록 하는 과정
- 부분적 종속성을 제거하는 것이 핵심. 이는 복합키의 일부분에만 종속되는 속성을 별도 테이블로 분리하는 과정
- 테이블에서 기본키가 복합키로 묶여있는 경우 두 키 중 하나의 키만으로 다른 컬럼을 결정지을 수 있으면 안된다.
  - 주문상세(주문ID, 제품ID, 제품이름, 제품카테고리, 주문수량, 단가) - 여기서 pk는 (주문ID, 제품ID)
    - 주문수량, 단가는 (주문ID, 제품ID) 전체에 종속됨 → 완전 함수적 종속
    - 제품이름, 제품카테고리는 제품ID에만 종속됨 → 부분 함수적 종속 (문제!)
      - 다음과 같이 변경
      - 주문상세(주문ID, 제품ID, 주문수량, 단가)
      - 제품(제품ID, 제품이름, 제품카테고리)
   
### 제3정규형(3NF)
2NF이면서 기본키를 제외한 속성들 간의 이행 종속성이 없어야 한다.
- 기본키가 아닌 속성들은 기본키에 의존해야 한다.
- A → B → C와 같은 이행적 종속성을 제거하는 것
- 직원(직원ID, 부서ID, 부서명, 부서위치)
  - 직원ID → 부서ID (직원ID가 부서ID를 결정)
  - 부서ID → 부서명, 부서위치 (부서ID가 부서명과 위치를 결정)
  - 따라서 직원ID → 부서ID → 부서명, 부서위치 (이행적 종속)
    - 다음과 같이 변경
    - 직원(직원ID, 부서ID)
    - 부서(부서ID, 부서명, 부서위치)

### BCNF
3NF이면서 모든 결정자가 후보키여야 한다.
- 수업(학생ID, 교수ID, 과목코드)
  - 교수ID → 과목코드
  - 그러나 교수ID는 후보키가 아님 (BCNF 위반)
    - 다음과 같이 변경
    - 교수과목(교수ID, 과목코드)
    - 수강신청(학생ID, 교수ID)
