...
- 테이블에서 기본키가 복합키로 묶여있는 경우 두 키 중 하나의 키만으로 다른 컬럼을 결정지을 수 있으면 안된다.
  - 주문상세(주문ID, 제품ID, 이름, 수량, 가격) - 여기서 pk는 (주문ID, 제품ID)
    - 다음과 같이 변경
    - 주문상세(주문상세(주문ID, 제품ID, 수량, 가격))
    - 제품(제품ID, 제품이름)
...
- 직원(직원ID, 부서ID, 부서명) - 여기서 부서명은 직원ID에 직접 종속되지 않고 부서ID를 통해 간접적으로 종속
  - 다음과 같이 변경
  - 직원(직원ID, 부서ID)
  - 부서(부서ID, 부서명)

### 코드 정규형(BCNF)
3NF이면서 모든 결정자가 후보키 집합에 속해야 한다.
- A → B, C와 같은 다중값 종속성을 제거
  - 학생과목(학생ID, 과목, 취미)
    - 다음과 같이 변경
    - 학생과목(학생ID, 과목)
    - 학생취미(학생ID, 취미)
